# SurveyAnnotate Produktkontext

## Zweck

SurveyAnnotate ist ein einmaliges Forschungswerkzeug zur Bewertung von Essay-Feedback. Annotatorinnen und Annotatoren erhalten persönliche Magic Links, lesen einen Schreibauftrag und ein Essay und bewerten drei anonymisierte Feedbacktexte mit festen siebenstufigen Likert-Skalen.

## Nutzer

- Studienleitung: richtet Gruppen, Materialien, Links und Exporte ein.
- Annotatorinnen und Annotatoren: bearbeiten ihre persönliche Aufgabenliste ohne Konto.

## Register

product

## Kernentscheidungen

- Es gibt eine einzelne Studie, keine Mehrmandanten- oder Mehrstudienverwaltung.
- Die UI ist vollständig deutsch.
- Adminschutz erfolgt über ein einzelnes Passwort.
- Teilnehmende werden manuell in Gruppen importiert.
- Es gibt drei Themen und sechs Gruppen; je zwei Gruppen werden einem Thema zugeordnet.
- Essays werden pro Thema gleichmäßig und ohne Überlappung auf die zwei Gruppen verteilt.
- Jeder Essay hat genau drei Feedbacktexte, deren Methodenidentität für Teilnehmende verborgen bleibt.
- Die Bewertungsfragen sind fest in der App hinterlegt: Verständlichkeit, Spezifität, Handlungsorientierung, Priorisierung, Bewältigbarkeit und Gesamt.
- Feedbackreihenfolgen werden im Entwurf randomisiert, können vor Aktivierung manuell sortiert werden und werden für Teilnehmende beim Start eingefroren.
- Antworten werden automatisch gespeichert; der Abschluss ist explizit und streng validiert.
- Nach Abschluss sehen Teilnehmende nur noch eine Abschlussseite.
- Exporte enthalten keine echten Namen, sondern nur Pseudonyme und numerische Likert-Werte.

## Erfolgsmaß

Die Studienleitung kann eine korrekte Studie importieren, validieren, aktivieren, Links kopieren, Fortschritt überwachen, Krippendorffs Alpha und Zusammenfassungen sehen und einen auswertbaren CSV-Export herunterladen. Teilnehmende können ohne Erklärung durch die Aufgaben navigieren, ihren Fortschritt sehen und nur vollständig abschließen.

## Nicht-Ziele

- Keine E-Mail-Zustellung.
- Keine Vollauthentifizierung mit externem Anbieter.
- Keine Unterstützung für mehrere Studien.
- Keine direkte Rankingfrage.
- Keine Änderung abgeschlossener Antworten durch Teilnehmende.
